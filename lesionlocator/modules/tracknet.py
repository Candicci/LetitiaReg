...
class TrackNet(nn.Module):
    ...
    def forward(self, x0, x1, prompt, is_inference=False, x1_mask=None, visualize=False, lesion_focused=False):
        ...
        if visualize:
            ...
            plt.tight_layout()
            plt.savefig(f'/home/xiachen/scripts/warped_vis/registration_input_resampled_{time}.png', dpi=150, bbox_inches='tight')
            plt.close()
        ...
        if visualize:
            ...
            plt.tight_layout()
            plt.savefig(f'/home/xiachen/scripts/warped_vis/registration_input_warped_{time}.png', dpi=150, bbox_inches='tight')
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(f'/home/xiachen/scripts/warped_vis/registration_input_warped_sagittal_{time}.png', dpi=150, bbox_inches='tight')
            plt.close()
        ...
        if visualize:
            ...
            plt.tight_layout()
            plt.savefig(f'/home/xiachen/scripts/warped_vis/registration_vis_slice_{slice_idx}_{int(time.time())}.png', dpi=150, bbox_inches='tight')
            plt.close()


            prompt_np = prompt.detach().cpu().numpy()[0]
            x1_mask_np = x1_mask.detach().cpu().numpy()[0]
            
            if np.sum(prompt_np > 0) > 0 and np.sum(x1_mask_np > 0) > 0:
            ...
            plt.title(f'Warped Prompt {mid_z} Slice,')
            ...
            plt.savefig(f'/home/xiachen/scripts/warped_vis/warped_prompt_slice_{mid_z}_{int(time.time())}.png')
            plt.close()
