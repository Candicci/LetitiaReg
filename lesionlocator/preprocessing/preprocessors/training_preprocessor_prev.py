    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], plans_manager: PlansManager,
        ...
        if bl_files:
            ...
            if not train:
                ...
                bl_seg, _ = rw.read_seg(bl_seg_file) if train and bl_seg_file is not None else None                
                # original_spacing = bl_data_properties['spacing']
                ...
                # median_size = [395, 256, 256]
                # bl_new_shape = compute_new_shape(bl_seg.shape[1:], original_spacing, self.target_spacing)
                # bl_spacing = (seg.shape[1]* original_spacing[0] / bl_seg.shape[1],
                #             seg.shape[2] * original_spacing[1] /  bl_seg.shape[2],
                #             seg.shape[3] * original_spacing[2] /  bl_seg.shape[3])
                ...
                bl_data, bl_seg, bl_data_properties = self.run_case_npy(bl_data, bl_seg, bl_data_properties, plans_manager, configuration_manager,
                                        dataset_json)
                # print(f"Resampling baseline data from shape {bl_data.shape} with spacing {original_spacing} to new shape {bl_new_shape} with spacing {bl_spacing}", flush=True)
                # bl_data = configuration_manager.resampling_fn_data(bl_data, bl_seg.shape[1:], original_spacing, bl_spacing)
                # bl_seg = configuration_manager.resampling_fn_seg(bl_seg, bl_seg.shape[1:], original_spacing, bl_spacing)
                ...
                if hasattr(configuration_manager, 'patch_size') and configuration_manager.patch_size is not None:
                    ...
                    # infer center point bl with bl_seg
    ...
    def _normalize(self, data: np.ndarray, seg: np.ndarray, configuration_manager: ConfigurationManager,
        ...
        for c in range(data.shape[0]):
            ...
            if scheme =='ZScoreNormalization':
                data[c] = normalizer.run(data[c], data[c] > 0.2)
            else:
