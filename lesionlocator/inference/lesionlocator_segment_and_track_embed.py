...
class LesionLocatorSegmenter(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            if isfile(join(model_track_training_output_dir, f'fold_{f}', 'best_tracking_model.pth')):
                ckpt_path_tracker = join(model_track_training_output_dir, f'fold_{f}', 'best_tracking_model.pth')
            # else:
            #     ckpt_path_tracker = join(model_track_training_output_dir, f'fold_{f}', "checkpoint_final.pth")
            ...
            # load decoder from segmentation model
    ...
    def predict_from_files(self,
        ...
        if os.path.isdir(source_folder_or_file):
            ...
            # output_files = [os.path.join(output_folder_or_file, os.path.basename(i)) for i in input_files]
            output_files = ['_'.join(os.path.basename(i).split('.')[0].split('_')[:3]) for i in input_files]

            finished_files = subfiles(output_folder_or_file, suffix=self.dataset_json['file_ending'], join=True, sort=True)
            ...
            if not os.path.isdir(output_folder_or_file):
                ...
                if not overwrite:
                    ...
                    # existing_files = [(os.path.isdir(i)) for i in output_files]
                    existing_files = [i for i in output_files if i in finished_output_files]
                    # not_existing_indices = [i for i, j in enumerate(input_files) if j not in existing_files]
                    not_existing_indices = [i for i, j in enumerate(input_files) if os.path.basename(j)[:-7] not in existing_files]
                    input_files = [input_files[i] for i in not_existing_indices]
                    prompt_files = [prompt_files[i] for i in not_existing_indices]
                    output_files = [os.path.join(output_folder_or_file, output_files[i]) for i in not_existing_indices]
